    ...
    <?php
        ...
        if ( ! empty( $products ) ) {
            ...
            if( $return_shop_button ){
                echo '<div class="htcompare-return-to-shop">
                    <p>Вы пока не добавили товаров для сравнения. Чтобы посмотреть и сравнить нужно перейти в каталог. При нажатии на кнопку «В сравнение» товары появятся  здесь.</p>
                   <a href="'.esc_url( wc_get_page_permalink( 'shop' ) ).'" class="button">'.esc_html( $return_shop_button ).'</a>
                </div>';
            }
        }
